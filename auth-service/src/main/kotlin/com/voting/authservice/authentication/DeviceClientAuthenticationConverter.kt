package com.voting.authservice.authentication
...
class DeviceClientAuthenticationConverter(deviceAuthorizationEndpointUri: String?) : AuthenticationConverter {
    ...
    init {
        val clientIdParameterMatcher =
            RequestMatcher { request: HttpServletRequest -> request.getParameter(OAuth2ParameterNames.CLIENT_ID) != null }
        ...
        this.deviceAccessTokenRequestMatcher = RequestMatcher { request: HttpServletRequest ->
            AuthorizationGrantType.DEVICE_CODE.value == request.getParameter(
                OAuth2ParameterNames.GRANT_TYPE
            ) && request.getParameter(OAuth2ParameterNames.DEVICE_CODE) != null && request.getParameter(
    ...
    override fun convert(request: HttpServletRequest): Authentication? {
        if (!deviceAuthorizationRequestMatcher.matches(request) && !deviceAccessTokenRequestMatcher.matches(request)) {
            return null
        }

        // client_id (REQUIRED)
        val clientId = request.getParameter(OAuth2ParameterNames.CLIENT_ID)
        if (!StringUtils.hasText(clientId) || request.getParameterValues(OAuth2ParameterNames.CLIENT_ID).size != 1) {
